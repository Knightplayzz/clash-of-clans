...
async function getClanCapitalRaidSeason(clanTag, limit) {
    ...
    if (clanTag.startsWith("#")) {
        ...
        if (!limit) {
            var fetchUrl = `https://api.clashofclans.com/v1/clans/${clanTagConverString}/capitalraidseasons`;
        } else {
            let limitTostring = limit.toString();
            var fetchUrl = `https://api.clashofclans.com/v1/clans/${clanTagConverString}/capitalraidseasons?limit=${limitTostring}`;
        }
