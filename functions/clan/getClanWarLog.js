...
async function getClanWarLog(clanTag, limit) {
    ...
    if (clanTag.startsWith("#")) {
        ...
        if (!limit) {
            var fetchUrl = `https://api.clashofclans.com/v1/clans/${clanTagConverString}/warlog`;
        } else {
            let limitTostring = limit.toString();
            var fetchUrl = `https://api.clashofclans.com/v1/clans/${clanTagConverString}/warlog?limit=${limitTostring}`;
        }
